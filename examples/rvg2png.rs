...
fn main() {
    ...
    for (group_id, group_props) in &model.groups {
        ...
        for prop in group_props {
            ...
            match *prop {
                FillColorRgba([r, g, b, a]) => fill_color = SRgba8::new(r, g, b, a),
                StrokeColorRgba([r, g, b, a]) => stroke_color = SRgba8::new(r, g, b, a),
        ...
        for pathop in &graphic.group[*group_id as usize] {
            match *pathop {
                ...
                rvg::PathOp::Move(a) => {
                    let (x, y) = (graphic.vertex_list[a as usize * 2], graphic.vertex_list[a as usize * 2 + 1]);
                ...
                rvg::PathOp::Quad(a, b) => {
                    let (bx, by) = (graphic.vertex_list[a as usize * 2], graphic.vertex_list[a as usize * 2 + 1]);
                    let (cx, cy) = (graphic.vertex_list[b as usize * 2], graphic.vertex_list[b as usize * 2 + 1]);
                    pathbuilder = pathbuilder.quad_to(bx, by, cx, cy);
                }
                rvg::PathOp::Cubic(a, b, c) => {
                    let (bx, by) = (graphic.vertex_list[a as usize * 2], graphic.vertex_list[a as usize * 2 + 1]);
                    let (cx, cy) = (graphic.vertex_list[b as usize * 2], graphic.vertex_list[b as usize * 2 + 1]);
                    let (dx, dy) = (graphic.vertex_list[c as usize * 2], graphic.vertex_list[c as usize * 2 + 1]);
        ...
        if fill_color.alpha() != Ch8::new(0u8) {
            raster.composite_matte((), p.fill(&path, footile::FillRule::NonZero), (), fill_color.convert(), SrcOver);
        }
        if stroke_color.alpha() != Ch8::new(0u8) {
            raster.composite_matte((), p.stroke(&path), (), stroke_color.convert(), SrcOver);
        }
    }
